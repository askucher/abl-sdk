...
angular.module('ablsdk').service('ablcalc', function($xabl, $timeout, p){
  ...
  return function(inputCharges, inputPayments){
    var payments, charges, makeEditable, byPrice, state, totalAdjustment, calcSubtotal, calcTaxFee, calcTaxesFees, showPrice, calcPrice, showAddonPrice, calcAddonPrice, totalAddons, calcCoupon, warning, calcTotal, calcPreviousTotal, calcBalanceDue, adjustment, coupon;
    ...
    warning = function(charge, name){
      var removed, aap, changed, res;
      removed = charge.status === 'inactive';
      aap = p.filter(function(it){
        return it.status === 'inactive' && it.name === charge.name && it.type === 'aap';
      })(
      inputCharges);
      changed = aap != null;
      res = (function(){
    ...
    adjustment = {
      list: p.filter(function(it){
        return it.type === 'adjustment' && it.status === 'active';
      })(
      payments),
      add: function(item){
        var newItem;
        ...
        newItem.amount *= 100;
        return adjustment.list.push(newItem);
      },
