...
angular.module('ablsdk').service('ablbook', function($xabl, p, stripe, debug, prefill, ablcalc){
  return function(activity, globalCallback){
    var state, getDay, investigateDate, valid, error, closeError, typing, resetIdenpotencyKey, few, sum, disabledOrder, cardify, expDateChanged, getEventInstanceId, cardChanged, stripeProcess, paymentSetup, checkout, agree, isError, showError, fields, tryCheckout, message, placeholder, x$;
    state = {
      triedCheckout: false,
      typing: '',
    ...
    getEventInstanceId = function(activity, calendar){
      var eventId;
      eventId = activity.timeslots.filter(function(it){
        return it._id === calendar._id;
      })[0].eventId;
      return eventId + '_' + calendar.date.origin;
    };
    ...
    stripeProcess = function(key, callback){
      ...
      return stripe.createToken(req, function(err, token){
        ...
        req = {
          ...
          eventInstanceId: getEventInstanceId(activity, state.calendar),
          addons: p.pairsToObj(
          p.map(function(a){
...
angular.module('ablsdk').service('ablcalc', function($xabl, $timeout){
  ...
  return function(inputCharges){
    var charges, makeEditable, byPrice, state, totalAdjustment, calcSubtotal, calcTaxFee, calcTaxesFees, showPrice, calcPrice, showAddonPrice, calcAddonPrice, totalAddons, calcCoupon, calcTotal, calcPreviousTotal, calcBalanceDue, adjustment, coupon;
    ...
    adjustment = {
      list: charges.filter(function(it){
