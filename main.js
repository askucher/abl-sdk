...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug, $xabl, $rootScope){
  return function(activity, inputModel){
    ...
    cutoff = (function(){
      ...
      case !(((ref5$ = activity.cutoff) != null ? ref5$ : 0) > 0):
        return activity.cutoff;
      default:
        return 48 * 60;
      }
    }());
    debug('cutoff', cutoff, (ref$ = $rootScope.user) != null ? ref$.preferences : void 8);
    inPast = function(date, flags){
      if (flags != null && flags.indexOf('include_nearest') > -1) {
        return getDay(date) < getDay(newDate());
      } else {
        return getDay(date) < getDay(newDate()) || date.diff(newDate(), 'minutes') < cutoff;
      }
    };
    ...
    x$.questions = (ref1$ = activity.questions) != null
      ? ref1$
      : [];
    x$.bg = activity.image;
    setup();
