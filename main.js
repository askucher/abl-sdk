  ...
  return function(url, initOptions){
    ...
    fetch = function(options){
      ...
      case 'Number':
        ...
        debug('number', i.options, options);
        break;
      case 'Object':
...
angular.module('ablsdk').service('ablbook', function($xabl, p, stripe, debug, prefill, ablcalc){
  return function(activity, globalCallback){
    var state, getDay, investigateDate, differentDays, valid, error, closeError, typing, resetIdenpotencyKey, few, moreThenOne, nobody, sum, disabledOrder, cardify, expDateChanged, cardChanged, stripeProcess, paymentSetup, checkout, agree, isError, showError, fields, tryCheckout, message, placeholder, x$;
    state = {
      triedCheckout: false,
      typing: '',
    ...
    differentDays = function(bag){
      return getDay(bag.start) !== getDay(bag.end);
    };
    valid = function(form){
    ...
    moreThenOne = function(){
      return few(state.calendar.attendees).length > 0;
    };
    nobody = function(){
      return state.calendar.attendees.filter(function(it){
        return it.quantity > 0;
      }).length === 0;
    };
    sum = function(arr){
    ...
    agree = function(){
      return state.form.agreed = !state.form.agreed;
    };
    isError = function(v){
    ...
    x$.differentDays = differentDays;
    x$.placeholder = placeholder;
    x$.closeError = closeError;
    x$.checkout = checkout;
    x$.agree = agree;
    x$.fields = fields;
    x$.tryCheckout = tryCheckout;
    x$.few = few;
    x$.moreThenOne = moreThenOne;
    x$.nobody = nobody;
...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug, $xabl){
  return function(activity, inputModel){
    ...
    makeAvailable = curry$(function(slot, arg){
      var available;
      available = slot.available - eval(([0].concat(model.calc.attendees.map(function(it){
        return it.quantity;
      }))).join('+'));
      return available;
    });
    performChooseSlot = function(slot){
      var day, merged, transform, ref$, ref1$, attendees, makeAttendee;
      if (slot.available === 0) {
        return;
      }
      day = model.value;
      ...
      model._id = slot._id;
      model.eventId = (ref$ = activity.timeslots.filter(function(it){
        return it._id === slot._id;
      })) != null ? (ref1$ = ref$[0]) != null ? ref1$.eventId : void 8 : void 8;
    ...
    loadEvents = function(callback){
      return ablapi.timeslots({
        ...
        slots.length = 0;
        loadedSlots.timeslots.map(transformDate).forEach(function(item){
    ...
    move = function(bookingId){
      return $xabl.put("bookings/move/" + bookingId, {
  ...
  init = function(){
    ...
    return model.createToken = function(card, callback){
      ...
      return c.createToken({
        ...
        state: (ref3$ = card.location) != null ? ref3$.state : void 8
