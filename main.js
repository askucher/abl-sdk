...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug){
  return function(activity, model){
    ...
    makeAvailable = curry$(function(slot, arg){
      var available;
      available = slot.available + 1 - eval(([0].concat(state.model.calc.attendees.map(function(it){
        return it.quantity;
      }))).join('+'));
      debug({
        available: slot.available + 1,
        taken: eval(([0].concat(state.model.calc.attendees.map(function(it){
          return it.quantity;
        }))).join('+')),
        result: available
      });
