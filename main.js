  ...
  return function(activity, inputModel){
    ...
    slotsByDay = function(day){
      ...
      transformSlot = function(slot){
        var start, duration, event, available, ref$, newSlot, x$;
        ...
        newSlot = angular.copy(slot);
        x$ = newSlot;
        x$.status = event != null ? event.status : void 8;
        x$.startTime = start;
        x$.time = start.valueOf();
        x$.endTime = start.clone().add(duration, 'milliseconds');
        x$.charges = slot.charges;
        x$.price = formula.getVisualPrice({
          timeslots: [slot]
        });
        x$.available = (event != null ? event.status : void 8) === 'inactive' ? 0 : available;
        x$._id = slot._id;
        x$.duration = moment.duration(duration).format("M[M] d[d] h[h] m[m]").replace(/((^| )0[a-z])|[ ]/ig, '');
        x$.taken = slot.maxOcc - available;
        return newSlot;
      };
    ...
    findChosenEvent = function(){
      var pairs, id, dateTransform, day, slot, visualSlot;
      ...
      slots);
      if (slot != null && !isDisabledDay(day)) {
        selectDay(day);
        visualSlot = p.find(function(it){
          return it._id === slot._id;
        })(
        activeSlots);
        debug('visual', visualSlot);
        return chooseSlot(visualSlot);
      }
    };
