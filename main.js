...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug, $xabl){
  return function(activity, inputModel){
    var model, transformCharge, state, x$, ref$, getDay, newDate, generateCalendar, getMonth, hackDate, merge, makeAvailable, performChooseSlot, actualEvent, isEmpty, slotsByDay, select, isFitToSlot, isNotFitToAnySlot, inPast, createMonth, startMonth, setCalendars, scroll, nextMonth, calendar, loadEvents, isDummy, isDisabledDay, selectDay, notSelected, disabledSlot, notAvailableSlot, close, chooseSlot, isActiveDay, isDisabledMonth, isActiveMonth, isCalendarUpDisabled, calendarUp, calendarDown, setup, move, createEventInstanceId, y$;
    ...
    makeAvailable = curry$(function(slot, arg){
      var available;
      available = slot.available - eval(([0].concat(state.model.calc.attendees.map(function(it){
        return it.quantity;
      }))).join('+'));
      return available;
    });
    performChooseSlot = function(slot){
      ...
      state.model.eventId = activity.timeslots.filter(function(it){
    ...
    setup = function(){
      return setCalendars(generateCalendar(startMonth.clone()), generateCalendar(startMonth.clone().add(1, 'month')), function(){
        return selectDay(state.model.value);
      });
    };
    ...
    createEventInstanceId = function(){
      var transform;
      transform = abldate(activity.timeZone);
      return state.model.eventId + '_' + transform.backendify(state.model.date.start).replace(/[\:-]/ig, '');
    };
    y$ = state;
    y$.move = move;
    y$.setup = setup;
    y$.createEventInstanceId = createEventInstanceId;
    y$.startMonth = startMonth;
    y$.setCalendars = setCalendars;
    y$.selectDay = selectDay;
    y$.generateCalendar = generateCalendar;
    y$.calendarUp = calendarUp;
    y$.calendarDown = calendarDown;
    y$.close = close;
    y$.isActiveMonth = isActiveMonth;
    y$.isDisabledDay = isDisabledDay;
    y$.isDisabledMonth = isDisabledMonth;
    y$.isCalendarUpDisabled = isCalendarUpDisabled;
    y$.isDummy = isDummy;
    y$.isActiveDay = isActiveDay;
    y$.notSelected = notSelected;
    y$.chooseSlot = chooseSlot;
    y$.notAvailableSlot = notAvailableSlot;
    y$.disabledSlot = disabledSlot;
    setup();
    return state;
  };
});
