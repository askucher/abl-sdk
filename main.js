...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug, $xabl){
  return function(activity, inputModel){
    ...
    findChosenEvent = function(){
      var ref$, pairs, id, dateTransform, day, slot, visualSlot, visualSlot2;
      ...
      if (slot != null) {
        ...
        if (!isDisabledDay(day)) {
          ...
          if (notAvailableSlot(slot)) {
            ...
            return observer.notify('sold-out');
          } else {
            return chooseSlot(visualSlot);
          }
        } else {
          visualSlot2 = p.find(function(it){
            return it._id === slot._id;
          })(
          activeSlots);
          debug('findevent-disabled', day, slot, visualSlot2);
          if (notAvailableSlot(visualSlot2)) {
            debug('findevent-not-available-slot', day, visualSlot2);
            observer.notify('sold-out');
          }
          if (inPast(day)) {
            debug('findevent-inpast', day);
            return observer.notify('too-close');
          }
        }
      }
    };
    ...
    observer = {
      ...
      notify: function(name, data){
        return observer.lish.each(function(watch){
