...
angular.module('ablsdk').service('ablbook', function($xabl, p, stripe, debug, prefill, safeApply){
  return function(activity, globalCallback){
    ...
    showErrorLogical = function(name, v){
      ...
      debug({
        "!s.touched and !state.tried-checkout": !s.touched && !state.triedCheckout,
        "s.active and !state.tried-checkout": s.active && !state.triedCheckout,
        "!s.active and s.touched": !s.active && s.touched,
        "state.tried-checkout": state.triedCheckout
      });
      if (show) {
    ...
    tryCheckout = function(){
      if (state.form.agreed) {
        debug('change-to-tried-checkout', 'try-checkout');
...
angular.module('ablsdk').service('ablcalc', function($xabl, $timeout, p, debug, test, typecheck){
  ...
  return function(inputNewCharges, inputPrevousCharges, paid){
    ...
    calcCoupon = function(){
      var code, subtotal, amount, _;
      code = coupon.codes[0];
      subtotal = calcSubtotal();
      amount = function(){
        switch (false) {
        case !(code.amount < subtotal):
          return coupon.codes[0].amount;
        default:
          return subtotal;
        }
      };
      _ = (function(){
        ...
        case code != null:
          return 0;
        case code.percentage !== true:
          return amount();
        case code.percentage !== false:
          return subtotal / 100 * coupon.codes[0].amount;
        default:
          return 0;
        }
      }());
...
angular.module('ablsdk').service('ablslot', function(abldate, ablcalc, ablapi, formula, p, debug, $xabl){
  return function(activity, inputModel){
    var transformCharge, getDay, newDate, generateCalendar, getMonth, hackDate, merge, makeAvailable, defineDateStart, performChooseSlot, actualEvent, isEmpty, slotsByDay, select, isFitToSlot, isNotFitToAnySlot, inPast, createMonth, startMonth, setCalendars, scroll, nextMonth, calendar, findChosenEvent, loadEvents, isDummy, isDisabledDay, selectDay, notSelected, disabledSlot, notAvailableSlot, close, chooseSlot, isActiveDay, isDisabledMonth, isActiveMonth, isCalendarUpDisabled, calendarUp, calendarDown, setup, move, eventInstanceId, createEventInstanceId, model, slots, calendars, activeSlots, x$, ref$, state, observer;
    ...
    performChooseSlot = function(slot){
      var day, transform, ref$, ref1$, attendees, makeAttendee;
      if (slot.available === 0) {
        return;
      }
      day = model.value;
    ...
    inPast = function(date){
      return getDay(date) < getDay(newDate()) || date.diff(newDate(), 'hours') < 48;
    };
