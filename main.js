...
angular.module('ablsdk').service('ablbook', function($xabl, p, stripe, debug, prefill){
  return function(activity, globalCallback){
    var state, getDay, investigateDate, valid, issue, error, closeError, typing, resetIdenpotencyKey, few, sum, disabledOrder, cardify, getEventInstanceId, stripeProcess, paymentSetup, validate, checkout, agree, isError, showErrorLogical, showError, fields, tryCheckout, message, placeholder, x$;
    state = {
      triedCheckout: false,
      typing: '',
    ...
    showErrorLogical = function(name, v){
      ...
      show = (function(){
        ...
        case !(!s.active && s.touched):
          return true;
        case !s.triedCheckout:
          return true;
        default:
          return false;
        }
      }());
      debug([["!s.touched and !state.tried-checkout", !s.touched && !state.triedCheckout], ["s.active and !state.tried-checkout", s.active && !state.triedCheckout], ["!s.active and s.touched", !s.active && s.touched], ["s.tried-checkout", s.triedCheckout]]);
      if (show) {
