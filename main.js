...
angular.module('ablsdk').service('crud', function($xabl, $rootScope, debug, $mdDialog, safeApply, watcher, p){
  return function(url, initOptions){
    var parsedUrl, state, frontendify, factory, provider, ref$, configureUrl, $scope, i, removeFromMemory, save, update, add, success, fetch, splice, remove, watchers, improve;
    parsedUrl = url.split('@');
    url = parsedUrl[0];
    state = [
      {
    ...
    i.converter = function(converter){
      ...
      return state.backendify = converter.backendify;
    };
...
angular.module('ablsdk').service('ablbook', function($xabl, p, stripe, debug, prefill, ablcalc){
  return function(activity, globalCallback){
    var state, getDay, investigateDate, valid, issue, error, closeError, typing, resetIdenpotencyKey, few, sum, disabledOrder, cardify, expDateChanged, getEventInstanceId, cardChanged, stripeProcess, paymentSetup, checkout, agree, isError, showError, fields, tryCheckout, message, placeholder, x$;
    state = {
      triedCheckout: false,
      typing: '',
...
// Generated by LiveScript 1.3.1
angular.module('ablsdk').service('ablcalc', function($xabl, $timeout, p){
  var sum;
  sum = function(arr){
    switch (false) {
    case typeof arr !== 'undefined':
  ...
  return function(inputCharges, inputPayments){
    var payments, charges, makeEditable, byPrice, state, totalAdjustment, calcSubtotal, calcTaxFee, calcTaxesFees, showPrice, calcPrice, showAddonPrice, calcAddonPrice, totalAddons, calcCoupon, calcTotal, calcPreviousTotal, calcBalanceDue, adjustment, coupon;
    payments = inputPayments != null
      ? inputPayments
      : [];
    ...
    calcPreviousTotal = function(){
      return calcTotal() - calcBalanceDue();
    };
    ...
    adjustment = {
      ...
      payments),
      add: function(item){
        return adjustment.list.push(item);
      },
  ...
  return function(activity, inputModel){
    ...
    makeAvailable = curry$(function(slot, arg){
      ...
      available = 'inactive' === slot.status
        ? 0
        : slot.available - eval(([0].concat(model.calc.attendees.map(function(it){
          return it.quantity;
        }))).join('+'));
      return available;
    });
    ...
    close = function(chosen){
      ...
      model.chosen = chosen;
      model.visible = false;
      if (typeof model.closed == 'function') {
        model.closed(chosen);
      }
      return debug('close', model);
    };
